...
while out_iters < 1:
    ...
    model = Model_lorenz(D_in,H1,D_out,dim,S,(D_in,),[4*H1,4*H1,1])
    i = 0
    # t = 0
    max_iters = 1000
    learning_rate = 0.01
    ...
    while i < max_iters:
        ...
        # loss = (theta-2)*((x*g)**2)+x**2*(2*x*f+g**2)
        # loss = (F.relu(loss)).mean()
    ...
    torch.save(torch.tensor(Loss), './data/loss_AIxz.pt')
    # torch.save(model._control.state_dict(),'./data/control_y_50_2000_2.5_leaf.pkl')
    model._control.load_state_dict(torch.load('./data/control_y_50_2000_2.5_leaf.pkl'))
    ...
    #     # cont_y = odeint(model.control_forward, test_y0, t, method='dopri5')[:,0,:]
