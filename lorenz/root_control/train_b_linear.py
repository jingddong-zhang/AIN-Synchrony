...
while out_iters < 15:
    ...
    max_iters = 1000
    learning_rate = 0.01
    optimizer = torch.optim.Adam([i for i in model.parameters()]+[model.delta_b], lr=learning_rate)
    Loss = []
    # theta = 0.75
    ...
    while i < max_iters:
        ...
        loss = -Vxg ** 2 / (V ** 2) + (2.+model.delta_b**2) * L_V / V
        ...
        i += 1
    b_list.append(2.+model.delta_b.item()**2)
    ...
    torch.save(best_model, './data/V_linear_{}_100_b.pkl'.format(out_iters+1))
    torch.save(torch.tensor(Loss),'./data/loss_{}_100_b.pt'.format(out_iters+1))
    ...
    out_iters += 1
np.save('./data/b_list_linear',np.array(b_list))
