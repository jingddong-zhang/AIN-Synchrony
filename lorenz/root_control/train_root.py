...
while out_iters < 1:
    ...
    hessV_data = torch.zeros([21,2000,3,2000,3])
    energy_list = []
    upara_L2 = []
    # n = 20
    # x,y,z = torch.linspace(-2.5,2.5,n),torch.linspace(-2.5,2.5,n),torch.linspace(-2.5,2.5,n)
    # X,Y,Z = torch.meshgrid(x,y,z)
    # sample_data = torch.stack([X, Y,Z], dim=3).view(-1,3).requires_grad_(True)
    ...
    while i <= max_iters:
        ...
        if i%100 == 0:
            def energy(data):
                ...
                t = np.linspace(0, 20, data.shape[1])
            ...
            hessV = torch.autograd.functional.hessian(V_dunc, x)
            hessV_data[int(i/100)] = hessV
            traj_data = generate(10,model,true_y0)
            rootC = np.zeros([traj_data.shape[0],traj_data.shape[1]])
            for l in range(traj_data.shape[0]):
                rootC[l] = model._control(traj_data[l,:, 1:6:2] - traj_data[l,:, 0:6:2])[:, 1].detach().numpy()
    ...
    torch.save(hessV_data, './data/V_hess_root_2000.pt')
    np.save('./data/energy_AI_root_traing_2000',np.array(energy_list))
